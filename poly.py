# This file was *autogenerated* from the file poly.sage.
from sage.all_cmdline import *   # import sage library
_sage_const_2 = Integer(2); _sage_const_1 = Integer(1); _sage_const_0 = Integer(0); _sage_const_6 = Integer(6); _sage_const_242 = Integer(242)
def calcula(p):
        if not(is_prime(p) and p%_sage_const_6 ==_sage_const_1 ):
                return
        d1 = ((p-_sage_const_1 )/_sage_const_2 )+_sage_const_1 
        d2 = ((p-_sage_const_1 )/_sage_const_6 )+_sage_const_1 
        count = _sage_const_0 
        
        for a in range(_sage_const_1 ,p):
                for b in range(_sage_const_1 ,p):
                         l = []
                         for x in range(_sage_const_0 ,p):
                                value = ( (x**d1) + (a*x**d2) + (b*x) ) % p
                                if l.count(value) == _sage_const_0 :
                                        l.append(value)
                         if len(l)==p:
                                count = count+_sage_const_1 
        print str(p)+", "+str(count)


for p in range(_sage_const_1 ,_sage_const_242 ):
        calcula(p)
