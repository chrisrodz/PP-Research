...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    \textbf{Construction}: If  $F_{a,b}(x) = x^{\frac{q+1}{2}} + a\cdot x^{\frac{q+1}{d}} + b\cdot x$ is a permutation polynomial for $a=\alpha^i$, $b=\alpha^j$ then $F_{a',b'}(x) = x^{\frac{q+1}{2}} + a'\cdot x^{\frac{q+1}{d}} + b'\cdot x$ is also a permutation polynomial for [$a',b'$] $\in \left\{ \alpha^{i+k\cdot (d+2)\cdot \frac{q-1}{2d}}, \alpha^{j+k\cdot \frac{q-1}{2}} \mid k=1,...,2d-1 \right\}$
  ...
  %   \begin{definition*}
  %     Let $F_{a,b}(x) = x^{\frac{q+1}{2}} + a\cdot x^{\frac{q+1}{d}} + b\cdot x$ be a polynomial defined over $\mathbb{F}_{q}$ where $d \mid q-1$. We define the sets $A_l = \left\{F_{a,b}(\alpha^{d\cdot k+i}) \mid k=0,...,\frac{q-1}{d}\right\}$ for $i=0,...,d-1$, where $\alpha$ is a primitive root of $\mathbb{F}_{q}$.
  %   \end{definition*}

  %   Using properties of these sets we will characterize $V_{F}$. First we would like to note that for $i \neq j$ the sets $A_i$ and $A_k$ are either equal, or distinct.
...
To characterize which polynomials $F_{a,b}(x)$ are permutation polynomials, we are studying the value sets of $F_{a,b}(x)$. We divide the value set into subsets:
  ...
  \begin{definition*}
    ...
    \begin{itemize}
      ...
      \item Study our results on the family of polynomials of the form $F_{a,b}(x) = x^{\frac{q+1}{2}+m} + a\cdot x^{\frac{q+1}{d}+m} + b\cdot x^{m}$
    \end{itemize}
