...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{itemize}
\item Verify if our results work for polynomials of the form $$(*)  F_{a,b}(x) = x^{\frac{q+1}{2}+m} + a\cdot x^{\frac{q+1}{d}+m} + b\cdot x^{m}$$ where $m \in \mathbb{F}_q$
\item Verify if there exist conditions for the pair $[a, b]$ such that $(*)$ \& $(**)$ are permutation polynomials. $$(**)  F_{a,b}(x) = x^{\frac{q+1}{2}} + a\cdot x^{\frac{q+1}{d}} + b\cdot x$$
\end{itemize}
   \vspace{0.3em}
  }\label{Future Work}
